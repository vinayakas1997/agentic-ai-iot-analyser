...
function renderComposedChart(cfg: ChartConfig, rows: Record<string, unknown>[]) {
  ...
  return (
    <ComposedChart data={rows}>
      ...
      <XAxis dataKey={cfg.xKey} {...axisLabelStyles()} />
      <YAxis {...axisLabelStyles()} />
...
function renderScatterChart(cfg: ChartConfig, rows: Record<string, unknown>[]) {
  return (
    <ScatterChart>
      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
      <XAxis dataKey={cfg.xKey} {...axisLabelStyles()} name={cfg.xKey} />
      <YAxis dataKey={cfg.yKeys[0] || ""} {...axisLabelStyles()} name={cfg.yKeys[0] || ""} />
...
function renderBasicChart(type: string, columns: string[], rows: Record<string, unknown>[]) {
  ...
  switch (type) {
    ...
    case "line":
      return (
        <LineChart data={rows}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
          <XAxis dataKey={xKey} {...axisLabelStyles()} />
          <YAxis {...axisLabelStyles()} />
          <Tooltip {...tooltipStyle()} />
          <Legend wrapperStyle={{ fontSize: 11 }} />
          {yKeys.map((k, i) => (
  ...
  return (
    <div ref={chartRef} className="mt-3 space-y-3">
      ...
      <div>
        <div className="flex gap-2 mb-2">
          {(["bar", "line", "area", "pie"] as const).map(t => (
            <button
              key={t}
              type="button"
              className={`text-[10px] font-medium px-2 py-0.5 rounded-full border transition-colors ${activeBasic === t ? "bg-accent text-white border-accent" : "text-muted border-border/50 hover:text-text"}`}
        ...
        {suggestions.basic.find(c => c.chartType === activeBasic)?.reason && (
        ...
        <div className="rounded-lg border border-border/50 bg-black/20 p-3">
          <ResponsiveContainer width="100%" height={250}>
            {renderBasicChart(activeBasic, columns, rows)}
          </ResponsiveContainer>
        </div>
      </div>
    </div>
