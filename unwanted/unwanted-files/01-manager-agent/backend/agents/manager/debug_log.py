"""Structured logging and debug helpers for the manager agent."""
...
def get_structured_metrics() -> dict:
    return dict(_METRICS)


def record_metric(name: str, value: float, tags: dict | None = None) -> None:
    entry = _METRICS.setdefault(name, {"count": 0, "total": 0.0, "min": float("inf"), "max": 0.0, "tags": tags or {}})
    entry["count"] += 1
    entry["total"] += value
...
def debug(node: str, message: str, **fields: Any) -> None:
    ...
    extra = {"node": node}
    if fields:
        extra["fields"] = {k: str(v)[:200] for k, v in fields.items()}
    logger.debug("[%s] %s", node, message, extra=extra)
    if fields:
        print(f"[DEBUG][{node}] {message} {json.dumps(fields, default=str)[:500]}")
    else:
