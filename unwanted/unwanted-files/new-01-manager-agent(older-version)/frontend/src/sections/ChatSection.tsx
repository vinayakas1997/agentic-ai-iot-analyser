...
import type { TurnUi, SuggestedAim } from "../types/manager";
...
function OptionCard({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      onSelect(`confirm ${confirmId || displayNumber}`);
    }
  };
  return (
    <div
      className={`bg-white/[0.02] border-2 rounded-lg p-3.5 mb-2.5 last:mb-0 cursor-pointer transition-all hover:-translate-y-px relative group ${
        isSelected
          ? "border-ic-violet/40 bg-ic-violet-soft/10 hover:bg-ic-violet-soft/20"
          : "border-border hover:border-stage-manager-line hover:bg-stage-manager-soft/20"
      }`}
      onClick={() => onSelect(`confirm ${confirmId || displayNumber}`)}
      onKeyDown={handleKeyDown}
      tabIndex={0}
      role="button"
      aria-label={`Select option ${displayNumber}: ${p.title || `Option ${displayNumber}`}`}
    >
      ...
      <div className="flex items-center gap-2 mb-1.5">
        ...
        {isSelected && (
          <span className="ml-1 text-[10px] font-semibold uppercase tracking-wider text-ic-violet">
            ✓ selected
          </span>
        )}
...
function formatUserMessage(turn: Turn): string {
  ...
  const match = text.match(/^confirm\s+(sug|cus|pro)-(\d+)$/i);
  if (match) {
    const fullId = `${match[1]}-${match[2]}`;
    const prefix = match[1];
    const proposals = turn.ui?.proposals || [];
    const suggestedAims = turn.ui?.suggested_aims || [];
    const customAims = turn.ui?.custom_aims || [];

    if (prefix === "pro") {
      const proposal = proposals.find(
        (p) => (p as Record<string, unknown>).confirm_id === fullId
      ) as Record<string, unknown> | undefined;
      if (proposal) {
        const title = (proposal.title as string) || "";
        return `user selected option ${fullId}: ${title}`;
      }
    } else if (prefix === "sug") {
      const sAim = suggestedAims.find((a) => a.confirm_id === fullId);
      if (sAim) {
        return `user selected option ${fullId}: ${sAim.aim}`;
      }
    } else if (prefix === "cus") {
      const cAim = customAims.find((a) => a.confirm_id === fullId);
      if (cAim) {
        return `user selected option ${fullId}: ${cAim.aim}`;
      }
    }
  }
  return text;
...
const COLORS = ["blue", "amber", "coral"] as const;
type Accent = (typeof COLORS)[number];

/* ── Resolve chip accent from dataset role; cycles for 10+ datasets ── */
function datasetAccent(
  ...
  const idx = datasets?.findIndex((d) => d.name === dsName) ?? -1;
  return idx >= 0 ? COLORS[idx % COLORS.length] : "amber";
}
...
export default function ChatSection() {
  ...
  const handleSuggestedAimClick = useCallback(async (confirmId: string, aimText: string) => {
    const store = useSessionStore.getState();
    const sid = store.sessionId;
    if (!sid) return;
    useUiStore.getState().selectTurn(turns.length - 1);
    store.setPendingTurn(aimText);
    try {
      const res = await sendMessage(sid, `confirm ${confirmId}`);
      const newTurn = {
        turn_index: res.turn_index ?? 0,
        user: `confirm ${confirmId}`,
      ...
      useSessionStore.setState((s) => ({
        ...
        pendingTurn: null,
        sessionMeta: {
          ...(s.sessionMeta || { session_id: sid }),
          phase: res.phase,
          status: res.status,
        },
      }));
      useUiStore.getState().selectTurn(useUiStore.getState().selectedTurnIndex + 1);
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : "Request failed";
      useSessionStore.setState({ error: msg, pendingTurn: null });
    }
  }, [turns.length]);
  ...
  }, [turns.length, loading, pendingTurn]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // keep input on failure so user can retry without retyping
    }
  };
              ...
              >
                ...
                {turn.agent && (
                  <div className={managerCardClass}>
                    ...
                    {/* Proposals cards — only when no proposal selected yet */}
                    {ui?.selected_proposal_index == null && (ui?.proposals?.length ?? 0) > 1 && (
                      <div>
                        ...
                        {ui.proposals.map((prop, pi) => {
                          const displayNumber = (prop as Record<string, unknown>).display_number as number ?? pi + 1;
                          const confirmId = (prop as Record<string, unknown>).confirm_id as string;
                          const isSelected = ui?.selected_proposal_index === (confirmId || displayNumber);
                          return (
                            <OptionCard
                              key={pi}
                              index={pi}
                              ...
                              onSelect={(msg) => {
                                const existingIdx = turns.findIndex(
                                  (t) => t.ui?.selected_proposal_index === (confirmId || displayNumber)
                                );
                                if (existingIdx >= 0) {
                                  scrollToTurn(existingIdx);
                                  return;
                                }
                                setActiveProposal({ index: pi, title: (prop.title as string) || `Option ${displayNumber}` });
                                useUiStore.getState().selectTurn(turns.length - 1);
                                sendUserMessage(msg);
                              }}
                            />
                    ...
                    {/* Markdown: selected proposal, single-proposal shortcut, or empty fallback */}
                    {(ui?.selected_proposal_index != null || (ui?.proposals?.length ?? 0) === 1 || ((ui?.plan?.aims?.length ?? 0) === 0 && (ui?.proposals?.length ?? 0) === 0)) && turn.agent && (
                    ...
                    {!ui?.plan?.aims?.length && !ui?.proposals?.length && ui?.suggested_aims?.length > 0 && (
                      <div className="mt-3 flex flex-col gap-4">
                        {(() => {
                          const singleAims = ui.suggested_aims.filter(a => !a.datasets || a.datasets.length <= 1);
                          const crossAims = ui.suggested_aims.filter(a => a.datasets && a.datasets.length > 1);

                          const groups: Record<string, SuggestedAim[]> = {};
                          for (const a of singleAims) {
                          ...
                          for (const [dsName, aims] of Object.entries(groups)) {
                            const accent = datasetAccent(dsName, schema?.datasets);
                            const dsRole = schema?.datasets?.find(d => d.name === dsName)?.role;
                            const groupKey = `single:${dsName}`;
                            const isExpanded = expandedGroups.has(groupKey);
                            const visibleAims = isExpanded ? aims : aims.slice(0, 2);
                            const hiddenCount = aims.length - 2;

                            rendered.push(
                              <div key={dsName}>
                                <div className={`text-[11px] font-semibold tracking-wider uppercase mb-2 ${chipClass(accent)} w-fit`}>
                                  <IconDatabase size={11} />
                                  {dsName}
                                  {dsRole && <span className="ml-1 opacity-70">({dsRole})</span>}
                                </div>
                                <div className="flex flex-col gap-2">
                                  {visibleAims.map((a) => {
                                    const pickedIdx = a.aim ? pickedAimTurnIndex[a.aim.trim()] : undefined;
                                    const isPicked = pickedIdx !== undefined;
                                    const confirmId = a.confirm_id || `sug-${a.display_number}`;
                                    return (
                                    <button
                                      key={a.aim || confirmId}
                                      type="button"
                                      onClick={() => {
                                        if (isPicked) {
                                          scrollToTurn(pickedIdx);
                                        } else {
                                          useUiStore.getState().selectTurn(turns.length - 1);
                                          handleSuggestedAimClick(confirmId, a.aim);
                                        }
                                      }}
                                      className={`w-full text-left px-4 py-3 text-sm rounded-lg border-2 transition-all duration-150 cursor-pointer ${
                                        isPicked
                                    ...
                                    >
                                      <div className="flex items-center gap-2 mb-1.5">
                                        <span className={`text-[10px] font-mono font-semibold px-1.5 py-0.5 rounded ${
                                          accent === "blue"
                                            ? "bg-ic-blue-soft/30 text-ic-blue"
                                            : accent === "amber"
                                              ? "bg-ic-amber-soft/30 text-ic-amber"
                                              : "bg-ic-coral-soft/30 text-ic-coral"
                                        }`}>
                                          {confirmId}
                                        </span>
                                        {isPicked && (
                                          <span className="ml-auto text-[10px] font-semibold uppercase tracking-wider text-ic-violet">
                                            ✓ asked
                                          </span>
                                        )}
                                      </div>
                                      <div className="text-text font-semibold mb-1">{a.aim}</div>
                                      {a.description && (
                                        <div className="text-muted text-[12.5px] leading-snug mb-1">{a.description}</div>
                                      )}
                                      {a.benefits && (
                                        <div className="text-[12px] text-ic-amber leading-snug">
                                          Benefit: {a.benefits}
                                        </div>
                                      )}
                                      {a.columns && a.columns.length > 0 && (
                                        <div className="mt-2 flex flex-col gap-0.5">
                                          {a.columns.map((colGroup) => (
                                            <div key={colGroup.dataset} className="text-[11px] text-text/80">
                                              <span className={monoClass}>{colGroup.dataset}:</span>{" "}
                                              <span className="text-text/90">{colGroup.names.join(", ")}</span>
                                            </div>
                                          ))}
                                        </div>
                                      )}
                                      {a.kpi_value && (
                                        <p className="text-[11px] text-muted mt-1 leading-snug">{a.kpi_value}</p>
                                      )}
                                    </button>
                                    );
                                  })}
                                  {!isExpanded && hiddenCount > 0 && (
                                    <button
                                      ...
                                      className={`w-full text-left px-4 py-2 text-[12px] font-semibold rounded-lg border-2 border-dashed transition-all cursor-pointer ${
                                        accent === "blue"
                                          ? "border-ic-blue/20 text-ic-blue hover:bg-ic-blue-soft/10"
                                          : accent === "amber"
                                            ? "border-ic-amber/20 text-ic-amber hover:bg-ic-amber-soft/10"
                                            : "border-ic-coral/20 text-ic-coral hover:bg-ic-coral-soft/10"
                                      }`}
                                    >
                                      +{hiddenCount} more
                                    </button>
                          ...
                          if (crossAims.length > 0) {
                            rendered.push(
                              <div key="__cross__">
                                <div className="flex items-center gap-2 mb-2">
                                  <span className={`${monoClass} text-[11px] font-semibold uppercase tracking-wider text-ic-violet bg-ic-violet-soft border border-ic-violet/30 px-2 py-0.5 rounded-[7px]`}>
                                    <IconDatabase size={11} className="inline mr-1" />
                                  ...
                                  {crossAims.map((a) => {
                                    ...
                                    <button
                                      ...
                                      className={`w-full text-left px-4 py-3 text-sm rounded-lg border-2 transition-all duration-150 cursor-pointer ${
                                        isPicked
                                          ? "bg-ic-violet-soft/15 border-ic-violet/40 hover:bg-ic-violet-soft/25"
                                          : "bg-ic-violet-soft/5 border-ic-violet/20 hover:bg-ic-violet-soft/15 hover:border-ic-violet/40"
                                      }`}
                                    >
                                      <div className="flex items-center gap-2 mb-1.5">
                                        <span className="text-[10px] font-mono font-semibold px-1.5 py-0.5 rounded bg-ic-violet-soft/30 text-ic-violet">
                                          {confirmId}
                                        </span>
                                        <div className="flex gap-1 flex-wrap">
                                          {a.datasets?.map((d) => {
                                            const dAccent = datasetAccent(d, schema?.datasets);
                                            return (
                        ...
                        <div className="border-t-2 border-border/20 pt-3 mt-1">
                          <p className="text-[12.5px] text-text/80 leading-relaxed">
                            <span className="font-semibold text-text">Have something specific in mind?</span>{" "}
                            Tell me what you want to analyze and I'll create a custom plan.
                          </p>
                        </div>
                      </div>
                    )}
            ...
            <div className="p-2.5 rounded-lg border-l-2 border-l-rail/30 bg-stripe-odd">
              <div className={userBubbleClass}>
                ...
                {(pendingTurn.schema?.line ||
                  ...
                  pendingTurn.schema?.no_time_filter) && (
                  <div className="mt-3 pt-2.5 border-t-2 border-border/30">
                    <span className="text-[10.5px] font-semibold tracking-wider text-tertiary uppercase block mb-1.5">
                      Resolved
                    </span>
