...
async def _start_research(event: Event) -> None:
...
Return ONLY a JSON array of query strings, e.g. ["df.shape", "df.describe()"]"""
    ...
    response = await complete(settings.research_model, [{"role": "user", "content": prompt}])
...
Return ONLY the fixed query string."""
    ...
    fixed = await complete(settings.research_model, [{"role": "user", "content": prompt}])
...
async def _finish_with_manager(event: Event, task_id: str, failed: bool = False, error: str = "") -> None:
    ...
    await publish_event("manager.result", event.user_id, base, session_id=event.session_id)
...
def register() -> None:
    subscribe("research.start", handle_start, settings.research_concurrency, "research_agent")
    subscribe("research.retry", handle_retry, settings.research_concurrency, "research_agent")
    subscribe("research.result", handle_result, settings.research_concurrency, "research_agent")
