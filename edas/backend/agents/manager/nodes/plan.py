...
from agents.manager.registry_context import build_research_schema_payload
...
async def _generate_plan_benefits(state: ManagerState, plan: dict) -> str:
    ...
    llm = _get_llm()
    response = await llm.ainvoke([SystemMessage(content=system), HumanMessage(content="Benefits:")])
...
async def detect_confirm(state: ManagerState) -> ManagerState:
    ...
    schema_payload = build_research_schema_payload(
...
async def send_to_planner(state: ManagerState) -> ManagerState:
    ...
    return {
        **state,
        ...
        "phase": "done",
        "agent_message": (
            "Ready for Planner Agent (CLI mode — not published to bus).\n\n"
            f"**Line:** {canonical}\n"
            f"**Aims:** {', '.join(aims)}\n\n"
            "Payload logged; enable bus integration in Phase 2."
        ),
    }
